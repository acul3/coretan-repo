...
class LocalizedNarrativesADE20kLoader(BaseLoaderWithDLManager):
    _ANNOTATION_URLs = {
        "train": _BASE_HF_URL + "ln_ade20k_train_captions_multi.jsonl",
        "validation": _BASE_HF_URL + "ln_ade20k_validation_captions_multi.jsonl",
    }
    ...
    def generate_gen_kwargs(self, dl_manager):
        annotation_file = dl_manager.download(self._ANNOTATION_URLs[self.split])
        image_base_dir = Path(dl_manager.download_and_extract(self._IMAGES_URL)) / "ADEChallengeData2016" / "images"
    ...
    def _build_rows_iterator(
        self, annotation_file: str, base_image_path: Path, chunk_size: int
    ) -> Iterator[List[Any]]:
        with open(annotation_file, "r", encoding="utf-8") as fi:
            yield from _file_to_chunks(fi, chunk_size)
    ...
    def _generate_examples(self, examples: List[Any], annotation_file: str, base_image_path: Path) -> Dict[str, Any]:
        ...
        return {
            "image_url": [None for _ in examples],
            "image": [str((base_image_path / f"{annotation['image_id']}.jpg")) for annotation in annotations],
            "text": [annotation["caption"] for annotation in annotations],
            "source": [self.source for _ in examples],
            "meta": [
                json.dumps(
...
class PMD(datasets.ArrowBasedBuilder):
    ...
    BUILDER_CONFIGS = [
        ...
        PMDConfig(name="localized_narratives_ade20k"),
    ...
    def _build_loaders(self, dl_manager, split_name):
        ...
        if self.config.name in ["localized_narratives", "localized_narratives_ade20k", "all"]:
            loaders.append(
